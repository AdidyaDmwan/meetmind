...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
        <div className="flex flex-col items-center mb-8">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h1 className="text-xl font-bold text-slate-900">MeetMind</h1>
          </div>
          <p className="text-slate-500 text-sm text-center">Create your account to start distilling insights</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <button type="submit" disabled={loading}
            className="w-full bg-slate-900 hover:bg-slate-700 text-white font-medium py-3 rounded-xl transition disabled:opacity-60">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button type="button" className="flex items-center justify-center gap-2 border border-slate-200 rounded-xl py-2.5 text-sm text-slate-600 hover:bg-slate-50 transition">
      ...
      <div className="fixed bottom-6 w-full flex justify-center text-xs text-slate-400">
        <span>© 2024 MEETMIND. BUILT FOR CLARITY.</span>
      </div>
    </div>
  );
}